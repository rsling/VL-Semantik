...
\title{Semantics{ }\\(6) Simply-typed higher order $\lambda$ languages}
\author{Roland Sch\"afer (University of G\"ottingen)}
\date{Summer Term 2005 (June 01)}
...
\begin{itemize}
  \item<1-> interpretation for [_S NP VP]:\\
     $\Dem{\ekm{_S\ NP\ VP}}{\mMm,g} = 1\ iff\ \Dem{NP}{\mMm,g}\in{}\Dem{VP}{\mMm,g}$
  \item<2-> Montague generally used CF's in definitions
  \item<3-> evaluating [_S [_{NP} \emph{Mary}] [_{VP} \emph{sleeps}] ] as a matter of \bl{functional application (FA)}: \\
     \begin{itemize}
...
\begin{itemize}
  \item<1-> all functions from S_1 to S_2
  \item<2-> \bl{$S_2^{S_1}$}
  \item<3-> for $T=\{0,1\}$ 
     \begin{itemize}
       \item<4-> $T^D$: all pred_1
       \item<5-> $T^{D\times{}D}$: all pred_2
     \end{itemize}
  \item<4-> 
\end{itemize}
}
...
\begin{itemize}
  ...
  \item<2-> semantic renaming of types:
     \begin{itemize}
       ...
       \item<5-> pred_1: \bl{$\ram{e,t}$}
       \item<6-> pred_2: \bl{$\ram{e,\ram{e,t}}$}       
     \end{itemize}
\end{itemize}
}
...
\begin{itemize}
  \item<1-> \bl{saturation} of complex types by FA:
    ...
    \end{itemize}
  \item<5-> for any pred_2 $P$ and its arguments $a_1,a_2$, $P(a_2)(a_1)$ is a wff
...
\begin{itemize}
  ...
  $h(k)(d)=1$ iff there is some $d^{\prime}\in D_{\ram{e}}$ s.t. k(d^{\prime})(d)=1
