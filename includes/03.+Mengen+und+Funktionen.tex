...
\frame{\frametitle{Set definition and elements: \textcolor{black}{$\in$}}
  \begin{itemize}
  \item<1-> M\Sub{1} = $\{a,b,c\}$
  \item<2-> N\Sub{1} = $\{$`my book'$\}$\\ vs. N\Sub{2} = $\{$my book$\}$ \\vs. N\Sub{3} = $\{$`my', `book'$\}$
  \item<3-> ill-formed: N\Sub{4} = $\{$`my', book$\}$
  \item<4-> defined by a property of its members:\\M\Sub{2} = $\{$x:x is one of the first three letters of the alphabet$\}$
  \item<5-> alternatively: \\M\Sub{2} = $\{$x$\|$x is one of the first three letters of the alphabet$\}$
...
\begin{itemize}
  ...
  \item<4-> Hence: \Up{*}Herr Webelhuth is numerous.
\end{itemize}
}
