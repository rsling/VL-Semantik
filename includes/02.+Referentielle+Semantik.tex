...
\begin{frame}
  ...
  \onslide<+->
  \begin{itemize}[<+->]
    \item[ ] Referenz\slash Denotat eines Ausdrucks A notiert als \alert{\den{A}}\\
      \grau{$\dem{\cdot}$ ist eine Funktion, nicht bloß eine Notationsweise.}
      \Zeile
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item[T] \alert{\textit{The truth value of \_ is 1.}}\\
      \grau{\footnotesize Lies $[_T A]$ als: \textit{Konstituente\slash Satz A eingebettet in den Kontext\slash Satz\slash die Konstituente T}}
      \Halbzeile
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item In $[\Sub{T}A]$ und $[\Sub{T}B]$ sind A und B jeweils in einer NP eingebettet,\\
      ...
      \gruen{\den{the truth value of A}$=$\den{the truth value of B}$=1$}
      \Halbzeile
    \item[ ] mit Ax2 daher \gruen{$\den{[\Sub{T}A]}=\den{[\Sub{T}B]}$}
    \item[ ] und damit \gruen{$\den{A}=\den{[\Sub{T}A]}=\den{[\Sub{T}B]}=\den{B}=1$}
      \Halbzeile
    \item Den Sätzen A und B ist aber nichts gemein außer ihrem Wahrheitswert.
    \item \gruen{Sätze referieren also auf Wahrheitswerte.}
...
\begin{frame}
  ...
  \onslide<+->
  \onslide<+->
  Nicht so \rot{sinnlos}, \rot{schwachsinnig} oder \rot{inhaltsleer} usw.\ wie oft vermutet\\
  \Zeile
  \begin{itemize}[<+->]
